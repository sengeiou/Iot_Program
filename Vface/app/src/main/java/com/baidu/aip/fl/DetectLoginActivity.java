...
public class DetectLoginActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (getIntent().getExtras() != null && !TextUtils.isEmpty(getIntent().getStringExtra("event_action"))) {
...
//                Color.parseColor("#3cf16d7a"));
                ...
                APIService.getInstance().identify(new OnResultListener<RegResult>() {
                    ...
                    public void onResult(RegResult result) {
                        ...
                        if (maxScore > 80) {
                            ...
                            startActivity(intent);*/
                            sendFaceBroadcast(true);
                            Toast.makeText(DetectLoginActivity.this, "人脸校验通过,人脸名：" + userInfo, Toast.LENGTH_SHORT).show();
                            //finish();
                            returnRegResult(true);
                            return;
                        } else {
                            Log.d("DetectLoginActivity", "onResult fail");
                            if (mDetectCount >= 3) {
                                mDetectTime = false;
                                sendFaceBroadcast(false);
                                Toast.makeText(DetectLoginActivity.this, "人脸校验不通过,请确认是否已注册", Toast.LENGTH_SHORT).show();
                                //finish();
                                returnRegResult(true);
                                return;
                    ...
                    public void onError(FaceError error) {
                        ...
                        intent.putExtra("DETECT_RESULT", false);
    ...
    public void returnRegResult(boolean isSuccess) {
        Intent intent = new Intent();
        intent.putExtra("DETECT_RESULT", isSuccess);
        setResult(RESULT_OK, intent);
        finish();
    }


    private void sendFaceBroadcast(boolean result){
