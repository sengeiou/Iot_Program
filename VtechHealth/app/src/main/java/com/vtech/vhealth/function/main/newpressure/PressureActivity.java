...
import com.vtech.vhealth.function.main.HealthAlertDialog;
...
public class PressureActivity extends BaseActivity<ACTPressureDelegate> implements View.OnClickListener  {
    // tag=1,准备测量，测量中，测量结束
    ...
    @Override
    protected Class<ACTPressureDelegate> getDelegateClass() {
        return ACTPressureDelegate.class;
    }
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        Bundle bundle = getContentResolver().call(URI, CASE_GET_COM, "", null);
        if (bundle != null) {
        ...
        deviceID= Utils.getDeviceId(PressureActivity.this);
        if (NetUtil.isConnected(PressureActivity.this)) {
            uploadToServer();
        }
        initDialog();
    }
    ...
    protected void bindEvenListener() {
        ...
        viewDelegate.getWave2().setGrayColor();
        viewDelegate.getWave3().setGreenColor();
    }
    ...
    public void onClick(View view) {
        switch (view.getId()) {
            case R.id.pressure_set_calibration:
                CalibrationActivity.start(this);
                break;
            case R.id.tv_start:
    ...
    private void handlerClick(String tag) {
        switch (tag) {
            case PressureFragment.TAG1:
                ...
                PlayVoiceUtil.play(this,getString(R.string.str_start_measure));
                break;
    ...
    private void startMeasure() {
        PressureActivity activity = viewDelegate.getActivity();
        if (activity != null) {
            ...
            if (activity != null && !mStop) {
                ...
                if (!TextUtils.isEmpty(key)) {
                    ...
                    if (TextUtils.isEmpty(deviceID)) {
                        healthDTOBean.setDeviceId(Utils.getDeviceId(PressureActivity.this));
                    }else {
...
//                    LogUtil.show(TAG, "healthDTOBean=" + healthDTOBean.toString());
    ...
    private void updateView(HealthDTOBean healthDTOBean, PressureActivity activity) {
...
//                saveRecord(health,"");
    ...
    private void autoStop(PressureActivity activity,HealthDTOBean healthDTOBean) {
        sendStopBroadcast(activity);
//        String voice = getString(R.string.str_health_stop, String.valueOf(healthDTOBean.getiSBP()),
//                String.valueOf(healthDTOBean.getiDBP()),String.valueOf(healthDTOBean.getiHRate()));
    ...
    private void sendStopBroadcast(PressureActivity activity) {
    ...
    private void uploadToServer(String health, final boolean save) {
        ...
        UserAPI.addHealths(health, new BodyCallback() {
